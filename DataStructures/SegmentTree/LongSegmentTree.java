/**
 * This is an abstract class, so you have to override the methods {@link #setIdentity()} and {@link #merge(long, long)}.
 ...
 */
abstract class LongSegmentTree {
    long[] tree;
    private final int n;
    long IDENTITY;
     ...
     * @param n    The length of the segment
     */
    public LongSegmentTree(java.util.function.IntToLongFunction init, int n) {
        this(n);
        for (int i = 0; i < n; i++)
            tree[this.n + i] = init.applyAsLong(i);
     ...
     */
    public void update(int position, java.util.function.LongUnaryOperator updateTo) {
        update(1, 0, n - 1, position, updateTo);
    }

    private void update(int v, int l, int r, int position, java.util.function.LongUnaryOperator updateTo) {
        if (position < l || position > r) return;
        if (l == r) {
            tree[v] = updateTo.applyAsLong(tree[v]);
            return;
        }
        int mid = (l + r) >> 1;
     ...
     */
    public int firstTrue(int l, int r, java.util.function.LongPredicate condition) {
        return firstTrue(1, 0, n - 1, l, r, condition);
    }

    private int firstTrue(int v, int l, int r, int ql, int qr, java.util.function.LongPredicate condition) {
     ...
     */
    public int lastTrue(int l, int r, java.util.function.LongPredicate condition) {
        return lastTrue(1, 0, n - 1, l, r, condition);
    }

    private int lastTrue(int v, int l, int r, int ql, int qr, java.util.function.LongPredicate condition) {
